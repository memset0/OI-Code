...
const int maxn = 500001, maxm = 201, maxk = 1001;
...
int get_sum2(int l, int r, int w) { return query(w, 1000, 1, 1, 1000, root[r], root[l - 1]); }
int get_cnt2(int l, int r, int w) { return query(w, 1000, 0, 1, 1000, root[r], root[l - 1]); }
...
void solve2() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        if (!~ans) puts("Poor QLW");
            ...
            now = get_sum2(a, b, ans);
            // printf("%d %d %d\n", ans, now - w, get_cnt(a, b, ans));
            ans = get_cnt2(a, b, ans) - (now - w) / ans;
            print(ans, '\n');
        }
    }
}
