...
node *merge(node *a, node *b) {
	return newnode(a->siz + b->siz, a->val, a, b);
}
...
void insert(int x, node *u = root) {
    if (u->siz == 1) {
        u->l = newnode(1, min(x, u->val), null, null);
        u->r = newnode(1, max(x, u->val), null, null);
    } else insert(x, x <= u->r->val ? u->l : u->r);
    update(u), maintain(u);
}
...
void erase(int x, node *u = root) {
    if (u->l->siz == 1 && u->l->val == x) {
        ...
        *u = *u->r;
        ...
        st[--cnt] = u->r;
        *u = *u->l;
    } else erase(x, x <= u->r->val ? u->l : u->r);
    update(u), maintain(u);
}
...
int getrank(int x, node *u = root) {
    if (u->siz == 1) return 1;
    return x <= u->r->val ? getrank(x, u->l) : getrank(x, u->r) + u->l->siz;
}
