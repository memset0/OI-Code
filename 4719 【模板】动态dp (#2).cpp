...
// 		for (int j = 0; j < 2; j++)
// 			for (int k = 0; k < 2; k++)
// 				c[i][j] = std::max(c[i][j], a.a[i][k] + b.a[k][j]);
// 	// putchar('{'), ~a, ~b, ~c, putchar('}');
// 	return c;
}
...
void main() {
    ...
    // for (int i = 1; i <= n; i++) printf("f[%d] : %d %d %d %d\n", i, f[i][0][0], f[i][0][1], f[i][1][0], f[i][1][1]);
    // for (int i = 1; i <= n; i++) printf("g[%d] : %d %d %d %d\n", i, g[i][0][0], g[i][0][1], g[i][1][0], g[i][1][1]);
    for (int i = 1; i <= m; i++) {
    ...
    // for (int i = 1; i <= n; i++) print(ch[i][0], " \n"[i == n]);
    // for (int i = 1; i <= n; i++) print(ch[i][1], " \n"[i == n]);
}
