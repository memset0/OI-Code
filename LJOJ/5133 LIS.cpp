...
namespace S2 {
	const int L = 15, M = N * 100;
	int tn, cnt, limit, rt[N], lc[M], rc[M], max[M], que[L], tmp[L];
	std::set < int > set;
	...
	int query(int u, int ql, int qr, int l, int r) {
		...
		return std::max(query(lc[u], ql, mid, l, mid),
			query(rc[u], mid + 1, qr, mid + 1, r));
	}

	void modify(int k, int w) {
		// printf("modify %d %d\n", k, w);
		for (int i = limit - k + 1; i <= n; i += i & -i)
			modify(rt[i], a[k], w, 1, tn);
	}

	int query(int k) {
		// printf("query %d %d\n", k, w);
		int ans = 0;
		for (int i = limit - k; i; i -= i & -i)
			maxd(ans, query(rt[i], a[k], tn, 1, tn));
		return ans;
	}
	...
	void update(int k) {
		...
		if (now != f[k]) {
			...
			if (now != f[k]) modify(k, now), f[k] = now;
			ans.insert(f[k]);
		}
	}
	...
	void init() {
		for (int i = 1; i <= m; i++) if (q[i].op == 1)
			b[++limit] = q[i].k;
		std::sort(b + 1, b + limit + 1);
		tn = std::unique(b + 1, b + limit + 1) - b - 1;
		for (int i = 1; i <= m; i++) if (q[i].op == 1)
			q[i].k = std::lower_bound(b + 1, b + limit + 1, q[i].k) - b;
		for (int i = 1; i <= m; i++) if (q[i].op == 1)
			b[++tn] = q[i].v - i;
		std::sort(b + 1, b + tn + 1);
